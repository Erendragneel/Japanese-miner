...
(function(){
  ...
  function renderProfile(){const player=document.getElementById('activePlayerName')?.textContent||'Miner';return `<div class="character-profile"><section class="character-preview-card"><div class="profile-nameplate"><span class="placement-kicker">Your miner</span><h3>${player}</h3><p>${state.selectedTitle||'Japanese Learner'}</p><strong class="cosmetic-balance">🪙 ${totalStoneValue().toLocaleString()} Nuggets</strong></div>${characterMarkup('large')}<div class="character-save-note">Skin tones are always free. Hair, clothes, footwear, and accessories are saved to this profile.</div></section><section class="character-customizer">${themeButtons()}${optionButtons('skin','Skin tone — free')}${optionButtons('hairStyle','Hair style')}${optionButtons('hairColor','Hair color')}${optionButtons('shirt','Clothing top')}${optionButtons('pants','Clothing bottom')}${footwearButtons('Footwear')}${optionButtons('accessory','Accessory')}<button id="randomizeCharacterBtn" class="primary" type="button">🎲 Randomize owned style</button></section></div>`;}
  ...
  window.renderJapaneseMinerCharacterShop=function(box){
    box.innerHTML=`<div class="shop-section-heading"><span>Character collection</span><h3>Hair, colors, clothing, footwear, and accessories</h3><p>Preview every locked style before spending Nuggets.</p></div><div class="character-profile shop-character-profile"><section class="character-preview-card">${characterMarkup('large')}</section><section class="character-customizer">${optionButtons('skin','Skin tones — free')}${optionButtons('hairStyle','Hair styles')}${optionButtons('hairColor','Hair colors')}${optionButtons('shirt','Clothing tops')}${optionButtons('pants','Clothing bottoms')}${footwearButtons('Footwear')}${optionButtons('accessory','Accessories')}</section></div>`;
    ...
    box.querySelectorAll('[data-footwear-value]').forEach(b=>b.addEventListener('click',()=>handleFootwearChoice(b,renderShop)));
  };
