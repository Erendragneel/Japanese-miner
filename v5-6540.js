/* Japanese Miner v6.5.4 — full-screen guardian trials and boot color collection. */
(function(){
'use strict';
const DAY=86400000;
const COMPANIONS=[
 ...
 ${stages.map((s,i)=>{
   ...
   return `<section class="world-region region-${i} ${i===selectedStageIndex()?'current-region':''} ${unlocked?'':'locked-region'}">
    ...
    <div class="world-road">${stageCategoryOrder(i).map((k,index)=>{
      ...
      return `${point>1?'<i class="road-piece"></i>':''}<button data-world-stage="${i}" data-world-point="${point}" data-world-category="${k}" class="world-node ${open?'open':'locked'} ${current?'current':''}" ${open?'':'disabled'}><span>${point}</span><small>${stageCategoryName(i,k)}</small><b class="world-node-progress">${stats[k].percent}%</b></button>`;
...
bossView=function(){
 ...
 return `<div class="v5-grid">${stages.map((s,i)=>{
   ...
   return `<article class="v5-card"><span class="v5-icon">${i===6?'🐉':'👹'}</span><h3>${esc(BOSSES[i])}</h3><p>Guardian of the ${esc(s.name)}. ${i<=1?'Complete all four kana alphabet groups':'Complete Vocabulary, Kanji, Grammar, and Reading'} before attempting the perfect 25-question trial.</p><button data-start-boss="${i}" ${!unlocked||defeated?'disabled':''}>${defeated?'Defeated':unlocked?'Start timed trial':'Complete all four sections'}</button></article>`;
 }).join('')}</div>`;
};
const BOSS_ICONS=['🗿','👻','👹','🐍','🥷','👺','🐉'];
let bossClock=null;
