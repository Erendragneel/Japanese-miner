// v6.5.2: full-screen guardian trials with live timer, question counter, and results.
...
(async()=>{
  try{
    const response=await fetch('cosmetics-6510.dat');
...
const gemTiers = [
 ...
 {name:"Amethyst", icon:"🟪", value:5, minStage:0, weight:40, desc:"Hiragana gemstone • required for the 5th heart."},
 {name:"Aquamarine", icon:"🔹", value:25, minStage:0, weight:30, desc:"Hiragana gemstone • required for the 6th heart."},
 {name:"Citrine", icon:"🟨", value:125, minStage:0, weight:22, desc:"Hiragana gemstone • required for the 7th heart."},
 {name:"Emerald", icon:"💚", value:625, minStage:1, weight:17, desc:"Katakana gemstone • required for the 8th heart."},
 {name:"Garnet", icon:"🔴", value:3125, minStage:1, weight:13, desc:"Katakana gemstone • required for the 9th heart."},
 {name:"Opal", icon:"🌈", value:15625, minStage:1, weight:10, desc:"Katakana gemstone • required for the 10th heart."},
 {name:"Ruby", icon:"❤️", value:78125, minStage:2, weight:7, desc:"JLPT N5 gemstone • required for the 11th heart."},
 {name:"Sapphire", icon:"💙", value:390625, minStage:3, weight:4.5, desc:"JLPT N4 gemstone • required for the 12th heart."},
 {name:"Topaz", icon:"🔶", value:1953125, minStage:4, weight:2.5, desc:"JLPT N3 gemstone • required for the 13th heart."},
 {name:"Red Diamond", icon:"♦️", value:9765625, minStage:5, weight:.45, desc:"The rarest gemstone • required for the final 14th heart."}
];
...
const DEFAULT_STATE = {
  ...
  selectedStage:0, soundEnabled:true, voiceEnabled:true, autoSpeak:true, voiceRate:.85, smartReview:true, sessionGoal:20, sessionAnswered:0, sessionCorrect:0, stageXp:[0,0,0,0,0,0,0], clearedStages:[], questionStats:{}, studyMinutes:{}, studySecondsCarry:0
};
...
function normalizeState(raw){
  ...
  next.studyMinutes=next.studyMinutes&&typeof next.studyMinutes==="object"?next.studyMinutes:{};
  Object.keys(next.studyMinutes).forEach(d=>next.studyMinutes[d]=Math.max(0,Number(next.studyMinutes[d])||0));
  next.studySecondsCarry=Math.max(0,Number(next.studySecondsCarry)||0);
  ...
  if(!next.gemBalanceV650Migrated){
    const retiredValues={Peridot:78125,Alexandrite:48828125,"Paraíba Tourmaline":244140625,Jadeite:1220703125};
    let retiredTotal=0;
    Object.entries(retiredValues).forEach(([name,value])=>{retiredTotal+=(Number(next.gemInventory[name])||0)*value;delete next.gemInventory[name];});
    if(retiredTotal>0) next.gemInventory.Agate=Number(next.gemInventory.Agate||0)+retiredTotal;
    next.gemBalanceV650Migrated=true;
  }
...
const HEART_UPGRADE_STONES=["Agate","Amethyst","Aquamarine","Citrine","Emerald","Garnet","Opal","Ruby","Sapphire","Topaz","Red Diamond"];
...
function mine(){
  ...
  // Expedition stops can request one learning category. Fall back to the full
  // stage pool when that stage does not yet contain a matching question.
  const expeditionCategory=String(window.JM_EXPEDITION_CATEGORY||"").toLowerCase();
  if(expeditionCategory){
    // Hiragana and Katakana mines are alphabet-only. Their four stops divide
    // the kana chart into character groups rather than vocabulary/grammar.
    if(idx===0||idx===1){
      const kanaRows={
        kana_basic_1:new Set(["a","i","u","e","o","ka","ki","ku","ke","ko","sa","shi","su","se","so"]),
        kana_basic_2:new Set(["ta","chi","tsu","te","to","na","ni","nu","ne","no","ha","hi","fu","he","ho"]),
        kana_basic_3:new Set(["ma","mi","mu","me","mo","ya","yu","yo","ra","ri","ru","re","ro","wa","wo","n"]),
        kana_voiced:new Set(["ga","gi","gu","ge","go","za","ji","zu","ze","zo","da","de","do","ba","bi","bu","be","bo","pa","pi","pu","pe","po"])
      };
      const wanted=kanaRows[expeditionCategory];
      if(wanted){
        const filtered=pool.filter(q=>{
          const answer=String(q?.a||"").toLowerCase().trim();
          return wanted.has(answer);
        });
      ...
      const categoryOf=q=>{
        ...
        const combined=`${k} ${id} ${String(q?.prompt||"").toLowerCase()}`;
        if(combined.includes("kanji"))return"kanji";
        if(combined.includes("grammar")||combined.includes("particle")||combined.includes("sentence")||combined.includes("dialogue")||combined.includes("verb")||combined.includes("adjective")||combined.includes("pattern"))return"grammar";
        if(combined.includes("reading")||k==="read")return"reading";
        if(combined.includes("vocab")||combined.includes("meaning")||combined.includes("conversation"))return"vocabulary";
        return"reading";
      };
      const filtered=pool.filter(q=>categoryOf(q)===expeditionCategory);
      if(filtered.length)pool=filtered;
    }
  }
  if(idx===2){
...
function showQuestion(q){
  const area=(state.v5?.boss&&document.getElementById("bossQuestionMount"))||document.getElementById("challengeArea");
...
(function(){
  ...
  const STUDIO_CATEGORIES=[
    ['skin','🧑','Skin'],['hair','💇','Hair'],['clothing','👕','Clothing'],['pants','👖','Pants'],
    ['boots','👢','Boots'],['gloves','🧤','Gloves'],['accessories','💍','Accessories'],
    ['pickaxe','⛏️','Pickaxe'],['companion','🐉','Companion'],['titles','🎭','Titles']
  ];
  const STUDIO_FASHION={
    clothing:[['none','No Jacket',0],['haori','Festival Haori',18000],['academy','Academy Blazer',22000],['explorer','Cave Explorer',30000]],
  ...
  function footwearOwned(value){
    ...
    return Array.isArray(state.v5?.ownedFashion)&&state.v5.ownedFashion.includes(`shoes:${value}`);
  }
  function footwearButtons(label='Footwear'){
    const equipped=state.v5?.fashion?.shoes||'boots';
    return `<div class="character-option footwear-option" data-character-section="footwear"><button type="button" class="character-option-toggle" aria-expanded="true"><span>👢 ${label}</span><b aria-hidden="true">⌄</b></button><div class="character-choice-grid footwear-choice-grid">${FOOTWEAR_OPTIONS.map(([value,name,price])=>{const owned=footwearOwned(value),selected=equipped===value;return `<button type="button" data-footwear-value="${value}" data-footwear-price="${price}" class="visual-choice footwear-choice ${selected?'selected':''} ${owned?'owned':'locked'}"><div class="choice-avatar-preview">${characterMarkup('mini',{_fashion:{shoes:value}})}</div><span>${name}<small>${selected?'Equipped':owned?'Owned':`${price.toLocaleString()} 🪙`}</small></span></button>`;}).join('')}</div></div>`;
  }
  function handleFootwearChoice(button,rerender){
    ...
    state.v5=state.v5||{};
    state.v5.fashion=Object.assign({jacket:'none',gloves:'none',shoes:'boots'},state.v5.fashion||{});
    state.v5.ownedFashion=Array.isArray(state.v5.ownedFashion)?state.v5.ownedFashion:['jacket:none','gloves:none','shoes:boots'];
    const token=`shoes:${value}`;
    if(!footwearOwned(value)){
      if(!spendStoneValue(price)){
        setMessage(`You need ${price.toLocaleString()} Nuggets for this footwear.`,'wrong');
        return;
      }
      if(!state.v5.ownedFashion.includes(token))state.v5.ownedFashion.push(token);
      setMessage(`${button.querySelector('span')?.childNodes[0]?.textContent?.trim()||'Footwear'} purchased and equipped!`,'correct');
    }else{
  ...
  function fashionOwned(key,value){
    ...
    return Array.isArray(state.v5?.ownedFashion)&&state.v5.ownedFashion.includes(`${key}:${value}`);
  }
  function studioFashionCards(key,items){
    const equipped=state.v5?.fashion?.[key]||(key==='shoes'?'boots':'none');
    return `<div class="studio-item-grid">${items.map(([value,name,price])=>{
      const owned=fashionOwned(key,value),selected=equipped===value;
  ...
  function accessoryPreview(value,name){
    const safeValue = ['none','glasses','headband','helmet','earrings','scarf'].includes(value) ? value : 'none';
    const src = `assets/avatar/accessories/${safeValue}.png?v=7.1.6`;
  ...
  function studioPickaxeCards(){
    return `<div class="studio-item-grid studio-equipment-grid">${PICKAXE_SKINS.map(item=>{
      const owned=state.ownedPickaxeSkins.includes(item.id),selected=state.equippedPickaxeSkin===item.id;
  ...
  function studioCompanionCards(){
    state.v5=state.v5||{};
    const ownedList=state.v5.ownedCompanions||['none'],equipped=state.v5.companion||'none';
    return `<div class="studio-item-grid studio-equipment-grid">${STUDIO_COMPANIONS.map(([id,name,icon,cost,bonus])=>{
      const owned=ownedList.includes(id),selected=equipped===id;
  ...
  function studioTitleCards(){
    ...
    (state.achievements?Object.keys(state.achievements):[]).forEach(id=>{
  ...
  function studioCategoryContent(){
    switch(activeStudioCategory){
      case 'skin': return `<div class="studio-section-heading"><h3>Skin</h3><p>Skin tones are always free.</p></div>${studioCharacterCards('skin',CHARACTER_OPTIONS.skin)}`;
      case 'hair': return `<div class="studio-section-heading"><h3>Hair</h3><p>Choose a hairstyle and hair color.</p></div><h4>Hair styles</h4>${studioCharacterCards('hairStyle',CHARACTER_OPTIONS.hairStyle)}<h4>Hair colors</h4>${studioCharacterCards('hairColor',CHARACTER_OPTIONS.hairColor)}`;
      case 'clothing': return `<div class="studio-section-heading"><h3>Clothing</h3><p>Choose a base top or layer a jacket over it.</p></div><h4>Base tops</h4>${studioCharacterCards('shirt',CHARACTER_OPTIONS.shirt)}<h4>Jackets</h4>${studioFashionCards('jacket',STUDIO_FASHION.clothing)}`;
      case 'pants': return `<div class="studio-section-heading"><h3>Pants</h3><p>Change your miner's lower outfit.</p></div>${studioCharacterCards('pants',CHARACTER_OPTIONS.pants)}`;
      case 'boots': return `<div class="studio-section-heading"><h3>Boots</h3><p>Preview, purchase, and equip footwear without leaving this screen.</p></div>${studioFashionCards('shoes',FOOTWEAR_OPTIONS)}`;
      case 'gloves': return `<div class="studio-section-heading"><h3>Gloves</h3><p>Equip practical or crystal mining gloves.</p></div>${studioFashionCards('gloves',STUDIO_FASHION.gloves)}`;
      case 'accessories': return `<div class="studio-section-heading"><h3>Accessories</h3><p>Add the finishing detail to your character.</p></div>${studioCharacterCards('accessory',CHARACTER_OPTIONS.accessory)}`;
      case 'pickaxe': return `<div class="studio-section-heading"><h3>Pickaxes</h3><p>Your equipped pickaxe appears throughout mining gameplay.</p></div>${studioPickaxeCards()}`;
      case 'companion': return `<div class="studio-section-heading"><h3>Companions</h3><p>Adopt a companion and choose who travels with you.</p></div>${studioCompanionCards()}`;
      case 'titles': return `<div class="studio-section-heading"><h3>Titles</h3><p>Display a title earned from your achievements.</p></div>${studioTitleCards()}`;
      default:return '';
    }
  }
  function renderCharacterStudio(){
    const player=document.getElementById('activePlayerName')?.textContent||'Miner';
  ...
  function handleStudioCharacter(button){
    const key=button.dataset.studioCharacterKey,value=button.dataset.studioCharacterValue,price=Number(button.dataset.studioPrice||0);
    if(!cosmeticOwned(key,value)){
      if(!spendStoneValue(price)){setMessage(`You need ${price.toLocaleString()} Nuggets.`,'wrong');return;}
      state.ownedCosmetics.push(cosmeticId(key,value));
    }
    state.character[key]=value;save();render();renderFeatureCenter('profile');
  }
  function handleStudioFashion(button){
    const key=button.dataset.studioFashionKey,value=button.dataset.studioFashionValue,price=Number(button.dataset.studioPrice||0);
    state.v5=state.v5||{};state.v5.fashion=Object.assign({jacket:'none',gloves:'none',shoes:'boots'},state.v5.fashion||{});
    state.v5.ownedFashion=Array.isArray(state.v5.ownedFashion)?state.v5.ownedFashion:['jacket:none','gloves:none','shoes:boots'];
    ...
    state.v5.fashion[key]=value;save();render();renderFeatureCenter('profile');
  }
  function handleStudioPickaxe(id){
    const item=PICKAXE_SKINS.find(x=>x.id===id);if(!item)return;
    if(!state.ownedPickaxeSkins.includes(id)){if(!spendStoneValue(item.cost)){setMessage(`You need ${item.cost.toLocaleString()} Nuggets.`,'wrong');return;}state.ownedPickaxeSkins.push(id);}
    state.equippedPickaxeSkin=id;save();render();renderFeatureCenter('profile');
  }
  function handleStudioCompanion(button){
    const id=button.dataset.studioCompanion,price=Number(button.dataset.studioPrice||0);
    state.v5=state.v5||{};state.v5.ownedCompanions=state.v5.ownedCompanions||['none'];
    if(!state.v5.ownedCompanions.includes(id)){if(!spendStoneValue(price)){setMessage(`You need ${price.toLocaleString()} Nuggets.`,'wrong');return;}state.v5.ownedCompanions.push(id);}
    state.v5.companion=id;save();render();renderFeatureCenter('profile');
  }
  function renderProfile(){return renderCharacterStudio();}
...
(function(){
  ...
  function studyDate(){return typeof dateKey==='function'?dateKey():new Date().toISOString().slice(0,10);}
  function ensureStudyClock(){
    state.studyMinutes=state.studyMinutes&&typeof state.studyMinutes==='object'?state.studyMinutes:{};
    state.studySecondsCarry=Math.max(0,Number(state.studySecondsCarry)||0);
  }
  function commitStudyTime(force=false){
    if(!activeProfileId||document.hidden){activeStudyTick=Date.now();return;}
    ensureStudyClock();
    const now=Date.now(),elapsed=Math.max(0,Math.min(70,(now-activeStudyTick)/1000));
    activeStudyTick=now;
    state.studySecondsCarry+=elapsed;
    const whole=Math.floor(state.studySecondsCarry/60);
    if(whole>0){const day=studyDate();state.studyMinutes[day]=Number(state.studyMinutes[day]||0)+whole;state.studySecondsCarry-=whole*60;if(force||whole>0)save();}
  }
  setInterval(()=>commitStudyTime(true),30000);
  document.addEventListener('visibilitychange',()=>{if(document.hidden)commitStudyTime(true);else activeStudyTick=Date.now();});
  window.addEventListener('beforeunload',()=>commitStudyTime(true));
  const priorMark=markPracticeToday;
  markPracticeToday=function(){priorMark();ensureStudyClock();const day=studyDate();if(!(day in state.studyMinutes))state.studyMinutes[day]=0;};
  const priorCalendar=renderStudyCalendar;
  renderStudyCalendar=function(){commitStudyTime(false);priorCalendar();ensureStudyClock();const total=Object.values(state.studyMinutes).reduce((a,b)=>a+Number(b||0),0);const stats=document.querySelector('.study-calendar-stats');if(stats&&!document.getElementById('calendarTotalMinutes'))stats.insertAdjacentHTML('beforeend',`<div><span>Total study time</span><strong id="calendarTotalMinutes">${Math.floor(total/60)}h ${total%60}m</strong></div>`);else if(document.getElementById('calendarTotalMinutes'))document.getElementById('calendarTotalMinutes').textContent=`${Math.floor(total/60)}h ${total%60}m`;document.querySelectorAll('#studyCalendarGrid .calendar-day[data-study-time-bound]').forEach(x=>x.removeAttribute('data-study-time-bound'));document.querySelectorAll('#studyCalendarGrid .calendar-day:not(.empty)').forEach(btn=>{if(btn.dataset.studyTimeBound)return;btn.dataset.studyTimeBound='1';btn.addEventListener('click',()=>{const y=studyCalendarMonth.getFullYear(),m=String(studyCalendarMonth.getMonth()+1).padStart(2,'0'),d=String(btn.textContent).padStart(2,'0'),key=`${y}-${m}-${d}`,mins=Number(state.studyMinutes[key]||0),detail=document.getElementById('calendarDayDetail');if(detail)detail.insertAdjacentHTML('beforeend',`<br><strong>Study time:</strong> ${Math.floor(mins/60)}h ${mins%60}m`);});});};

  function refreshMenuSummary(){
    const panel=document.querySelector('.game-menu-panel'),grid=panel?.querySelector('.game-menu-grid');if(!panel||!grid||typeof window.japaneseMinerCharacterMarkup!=='function')return;
    let summary=panel.querySelector('.menu-player-summary');if(!summary){summary=document.createElement('section');summary.className='menu-player-summary';grid.before(summary);}
    summary.innerHTML=`<div class="menu-quick-stats"><div><span>Nuggets</span><strong>🪙 ${totalStoneValue().toLocaleString()}</strong></div><div><span>Hearts</span><strong>❤️ ${state.hearts}/${state.maxHearts}</strong></div><div><span>Level</span><strong>⭐ ${state.level}</strong></div><div><span>Streak</span><strong>📅 ${state.practiceStreak}d</strong></div></div><div class="menu-player-character">${window.japaneseMinerCharacterMarkup('mini')}<div><span>Your miner</span><strong>${document.getElementById('activePlayerName')?.textContent||'Player'}</strong><small>${stages[selectedStageIndex()].name}</small></div></div>`;
    window.syncJapaneseMinerRenderedLayers&&summary.querySelectorAll('.miner-avatar').forEach(window.syncJapaneseMinerRenderedLayers);
  }
  const priorOpenMenu=openGameMenu;
  openGameMenu=function(){priorOpenMenu();refreshMenuSummary();};

  const BACK_MENU_OVERLAYS=[
    '#shopOverlay','#v5Overlay','#studyCalendarOverlay','#academyOverlay',
    '#featureCenterOverlay','#utilityOverlay','#statsDrawer',
    '#cosmeticPreviewOverlay','#bossTrialOverlay'
  ];
  ...
  function backToMenu(event){
    ...
    BACK_MENU_OVERLAYS.forEach(selector=>forceCloseMenuOverlay(document.querySelector(selector)));
    document.querySelectorAll('.utility-overlay.open,.preview-overlay.open,.boss-trial-page.open').forEach(forceCloseMenuOverlay);
  ...
  function addBackButton(root){
    if(!root)return;
    const head=root.querySelector('.shop-head,.v5-head,.study-calendar-head,.academy-head,.feature-center-head,.utility-head,.stats-drawer-head');
  ...
  const installBackButtons=()=>{
    ['#shopOverlay','#v5Overlay','#studyCalendarOverlay','#academyOverlay','#featureCenterOverlay','#utilityOverlay','#statsDrawer']
