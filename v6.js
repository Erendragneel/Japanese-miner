...
(()=>{
'use strict';
const VERSION='6.4.25-admin-feedback-notifications', SNAPSHOT_PREFIX='jm_v6_snapshots_', FEEDBACK_INBOX_KEY='jm_v6_admin_feedback_inbox', FEEDBACK_INBOX_LIMIT=200;
...
function openOverlay(name){shell();const o=overlay(name);o.classList.add('open');o.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';}
function closeOverlay(name){const o=overlay(name);o?.classList.remove('open');o?.setAttribute('aria-hidden','true');document.body.style.overflow='';}
let tourIndex=0;
...
function renderPlayerFeedback(box,notice=''){
 box.innerHTML=`${notice?`<div class="v6-feedback-sent" role="status">✓ ${esc6(notice)}</div>`:''}<p>Send feedback to the administrator profile on this device. A copy stays in this player profile and is included in account backups.</p><label>Category<select id="v6FeedbackCategory"><option>Something is confusing</option><option>Learning content</option><option>Balance or rewards</option><option>Visual or mobile issue</option><option>Feature idea</option></select></label><label>Message<textarea id="v6FeedbackText" rows="5" maxlength="800" placeholder="Tell the administrator what happened or what would improve the game..."></textarea></label><div class="v6-feedback-actions"><button id="v6SendFeedback" class="primary" type="button">Send Feedback</button><button id="v6ClearFeedback" type="button">Clear my history</button></div><h3>Your sent feedback (${state.v6.feedback.length})</h3><div class="v6-feedback-history">${state.v6.feedback.slice().reverse().map(x=>`<article><strong>${esc6(x.category)}</strong><p>${esc6(x.text)}</p><small>${new Date(x.at).toLocaleString()} · ${x.sentToAdmin===true?'Sent to admin':x.sentToAdmin===false?'Saved note':'Saved before admin inbox'}</small></article>`).join('')||'<p>No feedback sent yet.</p>'}</div>`;
 document.getElementById('v6SendFeedback').onclick=()=>{const text=document.getElementById('v6FeedbackText').value.trim();if(!text)return;submitPlayerFeedback(document.getElementById('v6FeedbackCategory').value,text);renderPlayerFeedback(box,'Feedback sent to the administrator.');};
