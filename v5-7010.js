/* Japanese Miner v7.0.1 — full-screen guardian trials and boot color collection. */
(function(){
'use strict';
const DAY=86400000;
const COMPANIONS=[
...
const KANA_CATEGORY_ORDER=['kana_basic_1','kana_basic_2','kana_basic_3','kana_voiced'];
...
const KANA_CATEGORY_NAMES={
 kana_basic_1:'Vowels, K & S',
 kana_basic_2:'T, N & H',
 kana_basic_3:'M, Y, R, W & N',
 kana_voiced:'Dakuten & Handakuten'
};
const KANA_CATEGORY_ICONS={kana_basic_1:'あ',kana_basic_2:'た',kana_basic_3:'ま',kana_voiced:'が'};
const CATEGORY_TARGET=20;
function stageCategoryOrder(stage){return stage<=1?KANA_CATEGORY_ORDER:CATEGORY_ORDER;}
function stageCategoryName(stage,key){return stage<=1?KANA_CATEGORY_NAMES[key]:CATEGORY_NAMES[key];}
function stageCategoryIcon(stage,key){return stage<=1?KANA_CATEGORY_ICONS[key]:CATEGORY_ICONS[key];}
...
function categoryQuestions(stage,category){
 ...
 if(stage<=1){
   ...
   const wanted=groups[category];
   return wanted?stagePool.filter(q=>wanted.has(String(q?.a||'').toLowerCase().trim())):[];
 }
...
function expeditionStats(stage){
 const order=stageCategoryOrder(stage);
 return Object.fromEntries(order.map(k=>[k,categoryProgress(stage,k)]));
}

checkpointCount=function(stage){
 const stats=expeditionStats(stage);
 let completed=0;
 for(const category of stageCategoryOrder(stage)){
...
bossUnlocked=function(stage){
 const stats=expeditionStats(stage);
 return stageCategoryOrder(stage).every(k=>stats[k].percent>=100);
};
...
mapView=function(){
 const scenery=['🌱','🎋','🌸','⛰️','❄️','🔥','🌌'];
 return `<div class="v5-hero world-hero"><div><span>Four-Skill Expedition</span><h3>${stages[selectedStageIndex()].name}</h3><p>${selectedStageIndex()<=1?'This alphabet mine contains kana-only practice. Its four stops divide the character chart into learning groups—no vocabulary, kanji, or grammar questions.':'This mine tracks Vocabulary, Kanji, Grammar, and Reading. Practice in each stop raises that section’s percentage.'} Complete all four to challenge the guardian.</p></div><b>🗺️</b></div>
 ...
 ${stages.map((s,i)=>{
   ...
   return `<section class="world-region region-${i} ${i===selectedStageIndex()?'current-region':''} ${unlocked?'':'locked-region'}">
    ...
    <div class="region-label"><strong>${esc(s.name)}</strong><span>${state.v5.bossDefeated.includes(i)?'Guardian cleared':`${stageCategoryOrder(i).filter(k=>stats[k].percent>=100).length}/4 sections complete`}</span></div>
    <div class="expedition-skill-grid">${stageCategoryOrder(i).map((k,index)=>`<div class="expedition-skill-chip"><span>${stageCategoryIcon(i,k)} ${stageCategoryName(i,k)}</span><b>${stats[k].percent}%</b><i><em style="width:${stats[k].percent}%"></em></i></div>`).join('')}</div>
    <div class="world-road">${stageCategoryOrder(i).map((k,index)=>{
      const point=index+1;
      const priorComplete=index===0||stageCategoryOrder(i).slice(0,index).every(previous=>stats[previous].percent>=100);
      const open=unlocked&&priorComplete;
      const current=open&&stats[k].percent<100;
      return `${point>1?'<i class="road-piece"></i>':''}<button data-world-stage="${i}" data-world-point="${point}" data-world-category="${k}" class="world-node ${open?'open':'locked'} ${current?'current':''}" ${open?'':'disabled'}><span>${point}</span><small>${stageCategoryName(i,k)}</small><b class="world-node-progress">${stats[k].percent>=100?'Replay':`${stats[k].percent}%`}</b></button>`;
    }).join('')}<i class="road-piece"></i><button data-world-stage="${i}" data-world-point="5" class="world-node ${unlocked&&bossUnlocked(i)?'open':'locked'}" ${unlocked&&bossUnlocked(i)?'':'disabled'}><span>🏯</span><small>${state.v5.bossDefeated.includes(i)?'Replay Boss':'Boss'}</small></button></div>
...
bossView=function(){
 ...
 return `<div class="v5-grid">${stages.map((s,i)=>{
   ...
   return `<article class="v5-card ${defeated?'boss-cleared-card':''}"><span class="v5-icon">${i===6?'🐉':'👹'}</span><h3>${esc(BOSSES[i])}</h3><p>Guardian of the ${esc(s.name)}. ${defeated?'This guardian has been cleared and may be challenged again at any time.':`${i<=1?'Complete all four kana alphabet groups':'Complete Vocabulary, Kanji, Grammar, and Reading'} before attempting the perfect 25-question trial.`}</p><button data-start-boss="${i}" ${!unlocked?'disabled':''}>${defeated?'Replay timed trial':unlocked?'Start timed trial':'Complete all four sections'}</button></article>`;
 }).join('')}</div>`;
};
const BOSS_ICONS=['🗿','👻','👹','🐍','🥷','👺','🐉'];
let bossClock=null;
...
function finishBossTrial(passed,reason){
 ...
 let bounty=0,firstClear=false;
 if(passed){
  firstClear=!state.v5.bossDefeated.includes(stage);
  if(firstClear)state.v5.bossDefeated.push(stage);
  state.v5.bossWins=Number(state.v5.bossWins||0)+1;
  state.v5.bossReplayWins=Number(state.v5.bossReplayWins||0)+(firstClear?0:1);
 ...
 card.innerHTML=`<div class="boss-result-icon">${passed?'🏆':'💥'}</div><span>${passed?'TRIAL PASSED':'TRIAL FAILED'}</span><h2>${passed?`${esc(BOSSES[stage])} ${firstClear?'Defeated':'Defeated Again'}`:'The Guardian Stands'}</h2><div class="boss-result-stats"><div><small>Correct</small><strong>${correct}/25</strong></div><div><small>Answered</small><strong>${answered}/25</strong></div><div><small>Time</small><strong>${(elapsed/1000).toFixed(1)}s</strong></div></div><p>${esc(reason)}</p>${passed?`<p class="boss-reward">+${bounty.toLocaleString()} Nuggets · ${firstClear?'Next mine unlocked':'Replay victory recorded'}</p>`:''}<div class="boss-result-actions"><button id="bossResultReturn" type="button">Return to Expedition</button><button id="bossResultRetry" class="primary" type="button">${passed?'Replay Trial':'Retry Trial'}</button></div>`;
