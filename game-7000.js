...
(function(){
  ...
  function backToMenu(){
    closeShop?.();closeV5?.();closeStudyCalendar?.();closeAcademy?.();closeFeatureCenter?.();setStatsDrawer?.(false);document.querySelectorAll('.utility-overlay.open,.preview-overlay.open').forEach(x=>x.classList.remove('open'));openGameMenu();
  }
  function addBackButton(root){
    if(!root||root.querySelector('.back-to-game-menu'))return;
    const head=root.querySelector('.shop-head,.v5-head,.study-calendar-head,.academy-head,.feature-center-head,.utility-head,.stats-drawer-head');if(!head)return;
    const b=document.createElement('button');b.type='button';b.className='back-to-game-menu';b.textContent='← Menu';b.setAttribute('aria-label','Back to game menu');b.addEventListener('click',backToMenu);head.prepend(b);
  }
  const navObserver=new MutationObserver(()=>{['#shopOverlay','#v5Overlay','#studyCalendarOverlay','#academyOverlay','#featureCenterOverlay','#utilityOverlay','#statsDrawer'].forEach(sel=>addBackButton(document.querySelector(sel)));});
  navObserver.observe(document.body,{childList:true,subtree:true});
  navObserver.takeRecords();
  ['#shopOverlay','#v5Overlay','#studyCalendarOverlay','#academyOverlay','#featureCenterOverlay','#utilityOverlay','#statsDrawer'].forEach(sel=>addBackButton(document.querySelector(sel)));
})();
